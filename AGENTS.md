...
`report.md`에는 중요 route 최대 50개를 도메인·경로 트리 형태의 사이트맵으로 표시한다. 사이트맵에는 Method, 파라미터, 중요도만 넣고 발견 소스는 넣지 않는다. 별도 중요 소스 정보에는 중요 엔드포인트 20개와 보안 관련 주석 10개만 표시한다. 원본 수집량은 줄이지 않으며 민감한 값은 보고서에서 가린다.
