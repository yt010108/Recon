...
- 스코프 정책이 허용하면 다음 단계는 자동으로 실행할 수 있다:
  ...
  - 프로빙(probe): httpx(HTTP 서비스 프로빙), robots.txt 확인, Nuclei(공식 서명 HTTP 템플릿만 사용한 발견 후보 수집)
...
- 이 프로젝트는 식별·열거·발견 후보 보고까지만 수행한다. Nuclei는 전용 어댑터의 고정된 공식 서명 HTTP 템플릿 중 `tech`·`exposure`·`misconfig` 태그만 사용하며 리다이렉트, OAST/Interactsh, DAST/퍼징, `dos`·`fuzz`·`intrusive` 태그, 코드·헤드리스 템플릿, 사용자 지정 템플릿을 허용하지 않는다. 그 밖의 익스플로잇, Metasploit 모듈, 자격 증명 스프레이, 퍼징 페이로드는 사용하지 않는다. 자동 탐지는 취약점 확정이 아니며 사람의 판단에 맡긴다.
