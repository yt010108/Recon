...
<style>.x { background:url("https://cdn.test/x"); } /* css raw */</style>"""
    ...
    def test_source_candidates_stay_in_scope_and_skip_binary(self) -> None:
        ...
        self.assertEqual(
            ...
            ["https://example.com/", "https://example.com/app.js?v=1"],
        )
...
class AdapterTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.policy = ScopePolicy.load(PROJECT_ROOT / "scopes" / "juice-shop.toml")
    ...
    def test_robots_adapter_keeps_comment_original_but_sanitizes_raw_log(self) -> None:
        ...
        outcome = ToolRunner(backend, self.store, PROJECT_ROOT).run_robots_txt(
            self.policy, self.state
        )
        parsed = json.loads(outcome.parsed_files[0].read_text(encoding="utf-8"))
        self.assertEqual(parsed[0]["body"], body)
        self.assertEqual(parsed[0]["comments"][0]["text"], " token=sample-original-value")
        self.assertNotIn("sample-original-value", outcome.raw_files[0].read_text(encoding="utf-8"))
    ...
    def test_source_adapter_keeps_exact_multiline_comment(self) -> None:
        ...
        backend = FakeBackend(result_with_record(record))
        outcome = ToolRunner(backend, self.store, PROJECT_ROOT).run_source_comments(
            self.policy, self.state
        )
        parsed = json.loads(outcome.parsed_files[0].read_text(encoding="utf-8"))
        self.assertEqual(parsed[0]["text"], " secret=VISIBLE\nsecond line ")
        self.assertNotIn("VISIBLE", outcome.raw_files[0].read_text(encoding="utf-8"))
        self.assertNotIn("-fr", backend.commands[0])
    ...
    def test_assetfinder_merges_sorted_unique_and_filters_scope(self) -> None:
        ...
        backend = FakeBackend(CommandResult(["assetfinder"], 0, stdout, ""))
        outcome = ToolRunner(backend, self.store, PROJECT_ROOT).run_assetfinder(
            policy, state
        )
    ...
    def test_subfinder_and_assetfinder_merge_into_one_hosts_file(self) -> None:
        policy = ScopePolicy.load(PROJECT_ROOT / "scopes" / "example.toml")
        state = self.store.create(policy.path, policy.snapshot())
        subfinder_out = CommandResult(
            ["subfinder"], 0, "c.example.com\n", ""
        )
        assetfinder_out = CommandResult(
            ["assetfinder"], 0, "c.example.com\nd.example.com\n", ""
        )
        runner = ToolRunner(FakeBackend(subfinder_out), self.store, PROJECT_ROOT)
    ...
    def test_amass_enum_is_passive_only_and_merges_hosts(self) -> None:
        ...
        backend = FakeBackend(
            CommandResult(["amass", "enum", "-passive", "-d", "example.com"], 0, stdout, "")
        )
        outcome = ToolRunner(backend, self.store, PROJECT_ROOT).run_amass_enum(policy, state)
        self.assertEqual(
            backend.commands[0],
